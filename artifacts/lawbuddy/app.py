...
# 2. Inicialização segura das variáveis de estado (Sem recarregamentos infinitos)
...
if not st.session_state.livro_aberto:
    ...
    # Botão limpo para abrir o livro
...
# --- FLUXO 2: LIVRO ABERTO (Leitura das Páginas) ---
    ...
    # Lógica de Paywall: Se passou da página 5 e não pagou, bloqueia tudo
    if num_pag > 5 and not st.session_state.comprado:
        ...
        if st.button("💳 COMPRAR ACESSO COMPLETO (19,99€)", use_container_width=True, type="primary"):
            st.session_state.comprado = True
            st.success("Compra simulada com sucesso! Divirta-se a ler.")
            st.rerun()
    ...
    # Se estiver dentro das páginas permitidas (ou se já comprou)
    else:
        # Botão do Spotify no topo
        st.link_button("🎵 Ligar Banda Sonora no Spotify", "https://spotify.com", use_container_width=True)
        
        st.divider()
        
        # Mostrar o conteúdo correspondente à página
        if num_pag in paginas_livro:
            st.header(paginas_livro[num_pag]["titulo"])
            st.write(paginas_livro[num_pag]["texto"])
        else:
            # Geração automática da história pós-compra até à página 70
            st.header(f"Capítulo {num_pag // 5 + 5}: A Fuga de Lisboa")
            st.write(f"Eva recolheu o seu portátil num movimento rápido, escapando pelas escadas de emergência enquanto os servidores do laboratório começavam a apagar os registos. Na página {num_pag}, ela sabia que cada passo seu estava a ser monitorizado pelo Chronos através das câmaras de vigilância da cidade. A música de suspense que ecoava nos seus auriculares dava o ritmo da sua fuga desesperada...")

        st.caption(f"Página {num_pag} de 70")
        st.divider()

        # Botões nativos de navegação de páginas (Funcionam 100% no telemóvel)
...
# --- BARRA LATERAL ADMINISTRATIVA (Apenas para ti, para poderes testar) ---
with st.sidebar:
    st.title("⚙️ Painel de Controlo")
    st.write("Usa as opções abaixo para simular o comportamento dos clientes:")
    
    # Ativa/Desativa o modo PRO (Pago)
    ...
    if st.button("Reiniciar e Fechar Livro"):
        st.session_state.livro_aberto = False
        st.session_state.pagina_atual = 1
        st.session_state.comprado = False
        st.rerun()
