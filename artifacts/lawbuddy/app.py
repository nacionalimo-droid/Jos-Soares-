...
st.set_page_config(page_title="O Último Algoritmo - Edição Completa", layout="centered")

# Inicialização das variáveis de controlo de leitura e pagamento
...
# --- REPRODUTOR DE MÚSICA NATIVO (PIANO DE SUSPENSE) ---
st.write("🎵 **Banda Sonora de Fundo (Piano Clássico)**")
st.audio("https://soundhelix.com", format="audio/mp3", loop=True)

# --- BASE DE DADOS COMPLETA DO LIVRO ---
book_data = {
    1: {"is_cover": True, "title": "O ÚLTIMO ALGORITMO", "subtitle": "A Mente Atrás da Máquina"},
...
# --- PROCESSAMENTO DAS PÁGINAS E PAYWALL ---
page = st.session_state.current_page

# Se passar da página 5 e não tiver pago, bloqueia a leitura
if page > 5 and not st.session_state.is_pro:
    ...
    # Renderização da folha do livro
    st.markdown('<div class="book-sheet"><div class="ribbon"></div>', unsafe_allow_html=True)
    
    if page == 1:
        # Página 1: Capa com Gráficos Embutidos
        st.markdown(f"""
        <div class="luxury-cover">
            <div class="cover-main-title">{book_data[1]['title']}</div>
            <div class="cover-sub-title">{book_data[1]['subtitle']}</div>
            
            <!-- Arte Geométrica do Olho -->
            <div class="art-eye"><div class="art-pupil"></div></div>
            
            <div class="cover-author-tag">Por Jos Soares</div>
        </div>
        """, unsafe_allow_html=True)
    else:
        # Páginas 2 a 5 (História Real) ou Páginas PRO 6 a 70 (História Estendida)
        if page in book_data:
            ...
            content = f"Eva Duarte apressou o passo pelas ruelas de Alfama na página {page}, sentindo o peso do relógio no pulso. Os acordes de piano que ouvira antes pareciam ecoar na névoa da noite. Cada linha de código que tentava reescrever no seu terminal portátil gerava um novo labirinto de decisões. O algoritmo Chronos continuava à espreita, calculando cada variante do seu plano de fuga..."
    ...
    # Botões de navegação por baixo da folha
    col1, col2 = st.columns(2)
    with col1:
        if page > 1:
            if st.button("⬅️ Voltar Página", use_container_width=True):
                st.session_state.current_page -= 1
                st.rerun()
    with col2:
        if page < 70:
...
# --- BARRA LATERAL DE TESTES (SIDEBAR) ---
with st.sidebar:
    st.title("⚙️ Administração da App")
